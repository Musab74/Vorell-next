			...
			<Stack width={'100%'} padding={'30px 24px'}>
				<Stack className={'profile'}>
					<Box component={'div'} className={'profile-img'}>
						<img
							src={member?.memberImage ? `${NEXT_APP_API_URL}/${member?.memberImage}` : '/img/profile/defaultUser.svg'}
							alt={'member-photo'}
						/>
					</Box>
					...
					<Stack className={'section'}>
						...
						<List className={'sub-section'}>
							{member?.memberType === 'STORE' && (
								<ListItem className={category === 'watches' ? 'focus' : ''}>
									<Link
										href={{
											pathname: '/member',
											query: { ...router.query, category: 'watches' },
										}}
										scroll={false}
										style={{ width: '100%' }}
									>
