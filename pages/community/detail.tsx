...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
  ...
  const {
    ...
    skip: !articleId, // fixed: only skip when there is no articleId
  ...
  const {
    ...
    skip: !searchFilter.search?.commentRefId, // keep: comments ref driven by searchFilter
    notifyOnNetworkStatusChange: true,
    onCompleted: (data: T) => {
  ...
  const createCommentHandler = async () => {
    try {
      ...
      if (comment.trim() === '') throw new Error('Please write something!');
  ...
  const updateButtonHandler = async (commentId: string, commentStatus?: CommentStatus.DELETE) => {
    try {
      ...
      await sweetMixinSuccessAlert(commentStatus ? 'Successfully deleted!' : 'Successfully updated!');
      await getCommentsRefetch({ input: searchFilter });
    } catch (error: any) {
      await sweetMixinErrorAlert(error.message);
    } finally {
  ...
  const likeArticleHandler = async (u: T, id: string) => {
    try {
  ...
  return (
    <div id="community-detail-page" className="veloura-community-detail">
      <div className="container">
        <Stack className="main-box">
          <Stack className="left-config">
            ...
            >
              <Tab value="FREE" label="General" className={`tab-button ${articleCategory === 'FREE' ? 'active' : ''}`} />
              <Tab value="RECOMMEND" label="Recommendations" className={`tab-button ${articleCategory === 'RECOMMEND' ? 'active' : ''}`} />
              <Tab value="NEWS" label="Watch News" className={`tab-button ${articleCategory === 'NEWS' ? 'active' : ''}`} />
              <Tab value="HUMOR" label="Humor" className={`tab-button ${articleCategory === 'HUMOR' ? 'active' : ''}`} />
            </Tabs>
          </Stack>
          ...
          <div className="community-detail-config">
            <Stack className="title-box">
              <Stack className="left">
                ...
                <Typography className="sub-title">Thoughtful discussion for haute horlogerie and design.</Typography>
              </Stack>
              <Button
                onClick={() =>
                  router.push({
            ...
            <div className="config">
              <Stack className="first-box-config">
                <Stack className="content-and-info">
                  ...
                  <Stack className="info">
                    ...
                    <Stack className="divider" />
                    <Stack className="icon-info">
                      {boardArticle?.articleComments && boardArticle?.articleComments > 0 ? <ChatIcon /> : <ChatBubbleOutlineRoundedIcon />}
              ...
              <Stack className="second-box-config" sx={{ borderBottom: total > 0 ? 'none' : '1px solid #eee', border: '1px solid #eee' }}>
              ...
              {total > 0 && <Stack className="comments"><Typography className="comments-title">Comments</Typography></Stack>}
                      ...
                      {commentData?.memberId === user?._id && (
                        <Stack className="buttons">
                          ...
                          >
                            ...
                            >
                              <Typography variant="h4" color="#b9b9b9">Update comment</Typography>
                              <Stack gap="20px">
                                <input
                                  autoFocus
                                ...
                                <Stack width="100%" flexDirection="row" justifyContent="space-between">
                                  ...
                                  <Stack sx={{ flexDirection: 'row', alignSelf: 'flex-end', gap: '10px' }}>
                                    ...
                                    <Button variant="contained" color="inherit" onClick={() => updateButtonHandler(updatedCommentId)}>
                                      Update
                                    </Button>
                                  </Stack>
                                </Stack>
              ...
              {total > 0 && (
                <Stack className="pagination-box">
                  <Pagination
                    count={Math.ceil(total / (searchFilter.limit || 1)) || 1}
