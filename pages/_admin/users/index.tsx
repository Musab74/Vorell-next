...
import { MemberUpdate } from '../../../libs/types/member/update.member';
...
const AdminUsers: NextPage = ({ initialInquiry, ...props }: any) => {
	...
	const menuIconClickHandler = (e: React.MouseEvent<HTMLElement>, key: string) => {
		setAnchorEl(prev => ({ ...prev, [key]: e.currentTarget }));
	  };

	const menuIconCloseHandler = (key?: string) => {
		if (!key) return setAnchorEl({});
